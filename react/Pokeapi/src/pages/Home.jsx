...
function Home() {
  ...
  return (
    <div className="bg-blue-600 p-4 md:p-8 min-h-screen flex flex-col items-center gap-12 ">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        {pokemons?.results?.map((pokemon, index) => (
          <div key={index} className="border p-4 border-gray-300/50 rounded shadow  flex flex-col justify-center items-center text-white lg:hover:scale-105 lg:hover:shadow-2xl ">
            ...
            <img src={getImg(pokemon?.url)} alt={pokemon.name} />
              <h2>{pokemon.name}</h2>
            </Link>
          </div>
        ))}
    ...
    <div className="flex justify-center items-center gap-6">
      <button className="bg-white rounded-xl text-blue-600 cursor-pointer px-4 py-2 disabled:bg-gray-600  " disabled={!pokemons?.previous} onClick={() => updateState(pages -1, pokemons.previous)}>Prev</button>
      <p className="text-white">{pages}</p>
      <button className="bg-white rounded-xl text-blue-600 cursor-pointer px-4 py-2 disabled:bg-gray-600  " disabled={!pokemons?.next} onClick={() => updateState(pages + 1, pokemons.next)}>Next</button>
    </div>
    </div>
  );
}
